...
def make_batch_onehot_labels(target, num_classes, batch_size):
    onehot_labels = torch.zeros((batch_size, num_classes), dtype=torch.float32).cuda()
    for i in range(batch_size):
        onehot_labels[i, target[i]] = 1.0
    return onehot_labels


def make_batch_soft_labels(all_soft_labels, target, num_classes, batch_size):
