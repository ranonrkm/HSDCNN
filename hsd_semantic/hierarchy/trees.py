...
def load_hierarchy(dataset):
    ...
    """
    if dataset.startswith('tiered-imagenet'):  
        fname = os.path.join(config.HIERARCHY.ROOT, dataset, "tiered_imagenet_tree.pkl")
    elif dataset in ["ilsvrc12", "imagenet"]:
        fname = os.path.join(config.HIERARCHY.ROOT, dataset, "imagenet_tree.pkl")
    elif dataset.startswith('inaturalist19'):
        fname = os.path.join(config.HIERARCHY.ROOT, dataset, "inaturalist19_tree.pkl")
    elif dataset.lower() == "cifar100":
        fname = os.path.join(config.HIERARCHY.ROOT, dataset, "cifar100_tree.pkl")
    elif dataset.lower() == "cifar10":
        fname = os.path.join(config.HIERARCHY.ROOT, dataset, "cifar10_tree.pkl")
    else:
...
def load_distances(dataset, dist_type, data_dir):
    ...
    with open(os.path.join(data_dir, dataset, "{}_{}_distances.pkl".format(dataset, dist_type).replace("-", "_")), "rb") as f:
